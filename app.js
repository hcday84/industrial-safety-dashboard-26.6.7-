...
// ── 자격증 선호도 (★1~5) ──────────────────────────────────────────────────────
// 2026-08 정정: 예전 버전은 이 점수가 Q-Net 통계연보·사람인/잡코리아 채용공고 데이터를
// "종합 산출"한 것처럼 설명했지만, 실제로는 그런 실시간 데이터 연동이 없다. reason 필드에
// 있던 "연 응시 OO만명" 같은 구체 수치도 WebSearch로 대조해보니 다수가 실제 통계와
// 2~10배 차이가 나 전부 제거했다(예: 전기기사 "연 20만명"→실제 1.5~2만명, 정보처리기사
// "40만명"→실제 약 14만명). 지금 이 순위는 법정 선임 의무·업계 활용도 등 공개적으로
// 확인 가능한 정성적 근거를 바탕으로 담당자가 직접 매긴 참고용 순위이며, 정밀한 통계
// 집계가 아니다. UI에도 이 사실을 명시한다.
const CERT_POPULARITY = {
  ...
  '공인중개사':         { stars: 4.0, tag: '창업·취업', reason: '부동산 법정 업무자격, 응시자 수가 많은 대표 전문자격' },
  ...
  'ITQ':                { stars: 4.0, tag: '학생·직장인', reason: '학교·공기업 채용 시 우대, 응시자층이 넓은 사무 자격증' },
  '한식조리기능사':     { stars: 4.0, tag: '창업·취업', reason: '외식업 창업·취업 필수, 조리 분야 대표 기능사' },
  '조주기능사':         { stars: 4.0, tag: '바텐더 자격', reason: '호텔·바·레스토랑 음료 서비스 필수 자격증' },
...
const ENG_TO_JAMO = {
  ...
  // 대문자: Shift로 겹자음/겹모음이 되는 키(Q,W,E,R,T,O,P)만 다른 자모, 나머지는 소문자와 동일
...
function renderBookCompareTable(certName, books, checkedTitles) {
  ...
  const rows = [
    ...
    { label: '평점(추정)', get: b => b.rating ? `${b.rating.toFixed(1)} (${b.reviews || 0}건)` : '-' },
  ];
  el.innerHTML = `
    ...
    <div class="cl-compare-scroll">
      ...
      <p class="books-hint" style="margin:8px 0 0;"><i class="fa-solid fa-circle-info"></i> 평점·리뷰수는 참고용 추정치로, 교보문고 실시간 집계와 다를 수 있습니다.</p>
    </div>`;
}
